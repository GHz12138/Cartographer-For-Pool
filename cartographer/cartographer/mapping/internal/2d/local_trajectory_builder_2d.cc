...
LocalTrajectoryBuilder2D::AddRangeData(
  ...
  // 抛弃掉所有测量距离小于配置min_range的hit点，并把那些超过配置max_range的测量点划归miss点
  ...
  // 以上内容是将Cartographer系统中原始的激光点云数据转换成占用栅格和插入器需要的RangeData类型的数据
  ...
  if (num_accumulated_ >= options_.num_accumulated_range_data()) {
    ...
    // std::cout << "local -> gravity_alignment: " << gravity_alignment.cast<float>() * range_data_poses.back().inverse() << std::endl;
    // std::cout << "range_data_poses.inverse(): " << range_data_poses.back().inverse() << std::endl;
    // std::cout << "gravity_alignment: " << gravity_alignment << std::endl;
